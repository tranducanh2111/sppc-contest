...
const Timeline = () => {
  ...
  useEffect(() => {
    const updateCardMargins = () => {
      // Check if the screen width is less than md (768px)
      ...
      // Apply dynamic margin-top for cards with index >= 2 only if screen is larger than md
      cardRefs.current.forEach((card, index) => {
        if (card) {
          card.style.marginTop = isSmallScreen || index < 1 ? '0' : `-${heights.current[index] / 2.5}px`;
        }
      });
    };
    ...
    return () => window.removeEventListener('resize', updateCardMargins);
  }, []);

  return (
    <section>
      <div className='mb-[3rem] md:mb-[6rem] flex flex-col gap-[0.75rem]'>
      ...
      <div className="relative">
        ...
        <div className="hidden md:block md:absolute left-1/2 transform -translate-x-1/2 h-full flex flex-col items-center z-50">
        ...
        {timelineData.map((item, index) => (
          ...
          >
            {/* Connecting Line */}
            <div className={`hidden md:block md:absolute ${index % 2 === 1 ? 'left-[calc(50%-1px)]' : 'right-[calc(50%-1px)]'} top-1/2 transform -translate-y-1/2 w-[35%] h-0.5 bg-black z-0`} />

            {/* Card Content */}
            <div className={`w-full md:w-1/2 ${index % 2 === 0 ? 'md:pr-16 lg:pr-32' : 'md:pl-16 lg:pl-32'} z-50`}>
              <div className="flex flex-col items-start text-left rounded-[12px] bg-[#FBFDFF] shadow-lg">
